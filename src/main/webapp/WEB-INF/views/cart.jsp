            ...
            <c:if test="${sessionScope.Cart.items !=null}">  
              <c:forEach var ="items" items="${sessionScope.Cart.items}">
                <tr>
                  ...
                  <td>
                    <button class="normal" data-variantid="${items.variant.variantID}" onclick="removeItem(this)">
    ...
    <section id="cart-add" class="section-p1">
      <div id="subtotal">
        <div class="d-flex flex-row">
          <p>
            <span class="fs-4"><fmt:message key="label.total"/>:</span>
            <strong id="TotalPrice">
              <fmt:formatNumber value="${sessionScope.Cart.price}" pattern="#,##0 VNĐ"/>
            </strong>
          </p>
        </div>
        <a href="${ctx }/cart/checkout" class="btn btn-success ${empty sessionScope.Cart.items ?'disabled':''}" aria-disabled="true">
          <fmt:message key="cart.proceed"/>
        </a>
      </div>
    </section>
